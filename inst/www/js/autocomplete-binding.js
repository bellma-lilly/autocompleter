...
function autocomplete(inp) {
  ...
  $(inp).on("input.autocompleteBinding", function (e) {
    ...
    cursorLocation = $el[0].selectionStart;
    beforeCursor = val.substring(0,cursorLocation);
    afterCursor = val.substring(cursorLocation);
    ...
    var startRegex = new RegExp(".*\b");
    var endRegex = new RegExp("\b");
    if(mode.toLowerCase() === "brackets"){
      startLoc = beforeCursor.lastIndexOf("{");
      endLoc = beforeCursor.lastIndexOf("}");
      prefix = val.substring(0,startLoc+1);
      suffix += "}" + afterCursor.substring(afterCursor.indexOf("}"));
      valUC = val.substring(startLoc+1,cursorLocation).toUpperCase();
    }else if(mode.toLowerCase() === "startswith"){
      startLoc = 0;
      endLoc = 0;
      valUC = val.substring(startLoc,cursorLocation).toUpperCase();
    }else{ //default
      try{
        rstr = "x"+reverseString(beforeCursor);
        searchResult = /x.*?\b/.exec(rstr);
        startLoc = rstr.length - searchResult[0].length;
        endLoc = startLoc;
        prefix = val.substring(0,startLoc);

        searchResult = /x.*\b/.exec("x"+afterCursor);
        suffix = afterCursor.substring(searchResult.index-1);

        valUC = val.substring(startLoc,cursorLocation).toUpperCase();

        if(cursorLocation <= startLoc){
          return;
        }
      } catch(error){
        //ignore
    ...
    var onClick = function (ce) {
      $el.val(prefix + $(ce.target).data("value") + suffix).trigger("change");
    };
    ...
    for (var i = 0; i < len; i++) {
      ...
      var pos = -1;
      //I'm not sure what is supposed to happen with contains
      if (mode === "contains") pos = labUC.indexOf(valUC);
      ...
      if (pos >= 0) {
        ...
        b.innerHTML = prefix + lab.substr(0, pos);
        b.innerHTML += "<strong>" + lab.substr(pos, valLen) + "</strong>";
        b.innerHTML += lab.substr(pos + valLen) + suffix;
